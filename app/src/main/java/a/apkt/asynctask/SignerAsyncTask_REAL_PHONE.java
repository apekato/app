...
public class SignerAsyncTask_REAL_PHONE extends AsyncTask<Void, Void, Boolean> {
    ...
    public SignerAsyncTask_REAL_PHONE(
            View mProgressView,
...
//            KeyStore keyStore = KeyStorePKCS12.loadKeystore(mainActivity, digCertInputStream, pwd);
            ...
            byte[] bytes = UtilsService.convertInputStreamToBytes(inputStream);
            String filePath = UtilsService.getFilePath(mainActivity, uri);
            byte[] fileBytes = UtilsService.filePathToBytes(mainActivity, filePath);
            String sf = UtilsService.sha256String(fileBytes);
            // line of code needed in asynctask
            byte[] fileSignedBytes = signer.doDetachedSign(bytes);
